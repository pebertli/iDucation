...
- (BOOL) prepareCombination:(HVTree*) combination withAtoms:(NSArray*) array
{
    ...
    for (int i = 0; i<[allChildren count]; i++) {
        ...
        IDCAtom* currentAtom = ((IDCAtomCombination*)current.value).atom;
           ...
           IDCAtom* p = ((IDCAtomCombination*)current.value).atom;
        p.delay = [self delayForNode:current withDelay:0];
        if(current.parent)
            p.hasParent = YES;
        p.angle = ((IDCAtomCombination*)current.value).angle;
...
- (double) delayForNode:(HVTreeNode*) node  withDelay:(double)delayAcc
{
    
    if(node.parent)
    {
        delayAcc+=1;
        return [self delayForNode:node.parent withDelay:delayAcc];
    }
