...
- (id)initWithElement: (NSString*) symbol scale:(float) scale{
    ...
    if (self) {
        ...
        spark = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, 162, 209)];
        spark.animationDuration = 20.3;
        spark.animationRepeatCount = 1;
        //spark.transform = CGAffineTransformMakeScale(0.8, 0.8);
        
        NSMutableArray* frames = [[NSMutableArray alloc] init];
        for (int i = 1; i<=6; i++) {
            NSString* frameName = [NSString stringWithFormat:@"spark_1_%d.png",i];
            UIImage* im = [UIImage imageNamed:frameName];
            
            [frames addObject:im];
        }
        spark.layer.anchorPoint = CGPointMake(0.5, 1.0);
        spark.animationImages = frames;
        //spark.backgroundColor = [UIColor colorWithRed:1 green:0 blue:0 alpha:0.5];
        spark.center = CGPointMake(0, 0);
...
- (void) doCombinationAnimation:(double) timeSinceLastUpdate
{
    ...
    if(state == IDCAtomStateCombinating && fabs(self.frame.origin.x-desiredPoint.x)<1 && fabs(self.frame.origin.y-desiredPoint.y)<1)
    {
        ...
        if(hasParent)
        {
            CGPoint pp =  originToCenter(CGPointMake(0, 0), self.frame);
            NSLog(@"%f %f %f",pp.x,pp.y, radiansToEuler(angle)-180);
            spark.transform = CGAffineTransformMakeRotation(angle-eulerToRadians(180));
            spark.center = pointAround(pp, self.frame.size.width/2, angle-eulerToRadians(180));
            NSLog(@"c %f %f",spark.center.x, spark.center.y);

            [spark startAnimating];
        }
    }
